...
func (fn toolRunnerFunc) ExecuteTools(ctx context.Context, req ToolRunRequest) (ToolRunResult, error) {
	messages := make([]protocol.ToolResultMessage, 0, len(req.ToolCalls))
	terminateCount := 0
	for _, call := range req.ToolCalls {
		if err := ctx.Err(); err != nil {
			return ToolRunResult{Messages: messages}, err
		}
		...
		if err != nil {
			...
			if errors.As(err, &updateErr) {
				return ToolRunResult{}, updateErr.err
			}
			if ctx.Err() != nil {
				return ToolRunResult{Messages: messages}, ctx.Err()
			}
...
func emitTestEvent(ctx context.Context, emit EventSink, event protocol.AgentEvent) error {
	if emit == nil {
		return nil
	}
