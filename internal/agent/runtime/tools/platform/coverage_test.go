...
func TestSkillToolLoadsEnabledSkillAndReportsUnavailableSkills(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("skills.NewStore() error = %v", err)
	}
	...
	if !strings.Contains(result, "<skill name=\"inspect\" location=\"") || !strings.Contains(result, "Inspect the service.") {
...
func TestRepoHelpers(t *testing.T) {
	...
	if got := truncateRepoTail("你好世界", 2); got != "...(前段已截断)\n世界" {
		t.Fatalf("truncateRepoTail() = %q", got)
	}
	if got := truncateRepoTail("short", 10); got != "short" {
		t.Fatalf("truncateRepoTail(short) = %q", got)
	}
