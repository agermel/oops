...
func NewSkillTool(store *skills.Store) (tool.InvokableTool, error) {
	return utils.InferTool("skill",
		"Load a specialized skill when the task at hand matches one of the available skills listed in the system prompt. "+
			...
			"The skill name must match one of the available ",
		func(ctx context.Context, input *skillInput) (string, error) {
			...
			return skills.FormatContent(skill), nil
		})
}
...
func NewTools(ops OpsData, store *skills.Store) ([]tool.InvokableTool, error) {
