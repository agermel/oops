...
func (s *FileStorage) Create(sessionID string, entries []Entry) error {
	...
	syncErr := syncRootDir(root)
	if err := errors.Join(removeErr, syncErr); err != nil {
		phase := "directory-sync"
		if removeErr != nil {
			phase = "temp-cleanup"
		}
		return &StorageCommitError{Operation: "create", Phase: phase, Committed: true, Err: err}
	}
	return nil
}
