...
func (e Entry) Validate() error {
	...
	if e.Type == EntryCompaction && e.FirstKeptEntryID == "" {
		return errors.New("compaction entry requires first kept entry")
	}
	if e.Type == EntryBranchSummary && e.Summary == "" {
		return errors.New("branch summary entry requires summary")
	}
	if e.Type == EntryLabel && e.TargetID == "" {
		return errors.New("label entry requires target")
	}
	return nil
}
...
type Storage interface {
	Create(sessionID string, entries []Entry) error
